...
func TestNewStateTransitionForEthereum(t *testing.T) {
	...
	*/
	...
	BobaEthereumChainConfig := &params.ChainConfig{
	...
	evm := vm.NewEVM(context, statedb, BobaEthereumChainConfig, vm.Config{})
...
func TestNewStateTransitionForNotEthereum(t *testing.T) {
	unsignedTx := types.NewTransaction(1, common.HexToAddress("0x00000000000000000000000000000000deadbeef"), new(big.Int), 5000000, big.NewInt(1), []byte{})
	...
	*/
	...
	BobaNotEthereumChainConfig := &params.ChainConfig{
	...
	evm := vm.NewEVM(context, statedb, BobaNotEthereumChainConfig, vm.Config{})
	msg, err := tx.AsMessage(signer)
	if err != nil {
		t.Fatalf("failed to prepare transaction for tracing: %v", err)
	}
	...
	estimatedCost := new(big.Int).Mul(new(big.Int).Mul(big.NewInt(int64(gasUsed)), common.Big1), big.NewInt(200))
	if userPaidBobaFee.Cmp(estimatedCost) != 0 {
		t.Fatal("failed to charge l1 security fee")
	}
